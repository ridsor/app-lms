...
$(document).ready(function () {
    $(".custom-file-upload")
        .toArray()
        .forEach((el, i) => {
            var $fileInput = $(el).parent().find("input[type='file']");
            const $preview = $fileInput.next();

            $(el).off("dragover dragleave drop");
            ...
            $(el).on("drop", function (e) {
                ...
                if (files.length) {
                    ...
                    showFilePreview(files, $preview, $fileInput, $(el));
                }
            });
            ...
            $fileInput.on("change", function () {
                if (this.files && this.files[0]) {
                    showFilePreview(this.files, $preview, $fileInput, $(el));
                }
            });
        });
});
...
function showFilePreview(files, preview, fileInput, dropArea) {
    ...
    preview.find(".btn-remove-file").on("click", function () {
        ...
        showFilePreview(fileInput[0].files, preview, fileInput, dropArea);
    });
}
